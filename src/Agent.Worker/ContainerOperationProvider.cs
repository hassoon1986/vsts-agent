...
#else
                string workingDirectory = Path.GetDirectoryName(executionContext.GetVariable(Constants.Variables.System.DefaultWorkingDirectory, isFilePath: true).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar));
                container.MountVolumes.Add(new MountVolume(workingDirectory, container.TranslateToContainerPath(workingDirectory)));
